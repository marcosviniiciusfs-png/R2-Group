...
function App() {
  ...
  const [data, setData] = useState<FormData>({ objetivo: '', valor: 250000, prazo: '', entrada: '', nome: '', whatsapp: '', cidade: '' })
  ...
  const valid = useMemo(() => {
    ...
    if (step === 1) return data.valor >= 30000 && Boolean(data.prazo) && Boolean(data.entrada)
    return data.nome.trim().split(' ').length > 1 && data.whatsapp.replace(/\D/g, '').length >= 10 && data.cidade.trim().length > 2
  }, [data, step])
  ...
  return (
    <main>
      <header className="header">
        ...
        <button className="header-cta" onClick={() => goTo('simulador')}>Simular agora <ArrowRight size={16} /></button>
      ...
      <footer><div className="footer-brand"><img src={logo} alt="R2 Group" /><span>R2 <b>GROUP</b></span></div><p>Resultados <i /> Estratégia <i /> Crescimento</p><small>© {new Date().getFullYear()} R2 Group. Todos os direitos reservados.</small></footer>
    </main>
  )
}
